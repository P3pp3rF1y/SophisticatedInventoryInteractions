...
public class ScreenInteractionInjector {
	...
	private boolean tryHandleTransferKeybind(Screen screen, InputConstants.Key inputKey) {
		...
		InventoryInteractionsPacketHandler.INSTANCE.sendToServer(new ContainerInteractionPayload(actionType, !Screen.hasShiftDown(), SortBy.NAME));
	...
	private Button buildTransferButton(int x, int y, InteractionActionType actionType, boolean toContainer) {
		...
		return new TransferButton(new Position(x, y), filterByContents -> InventoryInteractionsPacketHandler.INSTANCE.sendToServer(new ContainerInteractionPayload(actionType, filterByContents, SortBy.NAME)), filteredDefinition, allDefinition);
	}
	...
	private static class TransferButton extends Button {
		...
		private TransferButton(Position position, Consumer<Boolean> transferAction, ButtonDefinition filteredDefinition, ButtonDefinition allDefinition) {
			super(position, filteredDefinition, mouseButton -> {
				if (mouseButton == 0) {
					transferAction.accept(!Screen.hasShiftDown());
				}
			});
		...
		protected List<Component> getTooltip() {
			return Screen.hasShiftDown() ? allDefinition.getTooltip() : filteredDefinition.getTooltip();
		}
	}
