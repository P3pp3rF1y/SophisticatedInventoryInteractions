...
public class ScreenInteractionInjector {
	...
	public void onScreenInit(ScreenEvent.Init.Post event) {
		...
		ClientPacketDistributor.sendToServer(new RequestSortMemoryPayload());
	...
	private void readPixelToSampleBuffer(int fbX, int fbY) {
		...
		cachedSampledPixelColor = 0xFF000000 | (r << 16) | (g << 8) | b;
	}
	...
	private boolean tryHandleTransferKeybind(Screen screen, InputConstants.Key inputKey) {
		...
		ClientPacketDistributor.sendToServer(new ContainerInteractionPayload(actionType, !Screen.hasShiftDown(), SortBy.NAME));
