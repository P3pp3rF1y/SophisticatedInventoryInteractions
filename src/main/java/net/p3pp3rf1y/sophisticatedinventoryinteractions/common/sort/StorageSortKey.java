...
public record StorageSortKey(Type type, ResourceLocation dimension, @Nullable BlockPos blockPos, @Nullable UUID entityId, @Nullable String compoundId) {
	...
	public static Optional<StorageSortKey> deserialize(CompoundTag tag) {
		...
		ResourceLocation dimension = tag.getString(DIMENSION_TAG).map(ResourceLocation::tryParse).orElse(null);
		if (dimension == null) {
			return Optional.empty();
		}

		return switch (type) {
			case BLOCK -> tag.getLong(BLOCK_POS_TAG).map(blockPos -> block(dimension, BlockPos.of(blockPos)));
